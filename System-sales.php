...
<html lang="en">
  ...
  <body>
            ...
            <div class="col-lg-4 mx-auto">
              <div class="auth-form-light text-left p-5">
                ...
                <form class="pt-3" action="" method="POST">
                  ...
                  <div class="form-group">
                    <input type="number" class="form-control form-control-lg" id="exampleInputPassword1" name="price" placeholder="Price" required/>
                  </div>
                  <div class="form-group">
                    <input type="date" class="form-control form-control-lg" id="exampleInputPassword2" name="date" placeholder="Date" required/>
                  </div>
                  <div class="mt-3">
                  <button type="submit" name="submit" class="btn btn-block btn-lg btn-gradient-primary mt-4">Record sale</button>
                  </div>
                </form>
                <?php
   if(isset($_POST["submit"])){
      ...
      $Price = $_POST['price'];
		  $Date =$_POST['date'];
      $query1= "INSERT INTO sales (id, productName, category, quantity, price, entryDate )
		  VALUES ('','$Product_name','$Category',$Quantity,$Price, '$Date')";
      $query2=$connection->prepare("SELECT quantity FROM stock WHERE productName=? and category=?");
      $query2->bind_param('ss',$Product_name,$Category);
      $query2->execute();
      $result=$query2->get_result();
      while($row = $result->fetch_assoc()){
        if($row["quantity"]<=0){
          echo "No such product in stock";
        }else{
        $UpdatedQTY=$row["quantity"]-$Quantity;
        if($UpdatedQTY<0){
          echo "Insufficient Quantity of Products";
        }else{
          $SQL=$connection->prepare("UPDATE stock set quantity=? WHERE productName=? and category=?");
          $SQL->bind_param('sss',$UpdatedQTY,$Product_name,$Category);
          $SQL->execute();

          $query3=$connection->prepare("SELECT cost FROM stock WHERE productName=? and category=?");
          $query3->bind_param('ss',$Product_name,$Category);
          $query3->execute();
          $result=$query3->get_result();
      while($row = $result->fetch_assoc()){
        $UpdatedStock=$row["cost"]-$Price;
      }
    }
      $r1 = mysqli_query($connection, $query1);
      
      $StockCost=$connection->prepare("UPDATE stock set cost=? WHERE productName=? and category=?");
      $StockCost->bind_param('sss',$UpdatedStock,$Product_name,$Category);
      $StockCost->execute();
      if(!$r1){
      ...
      $dlt=mysqli_query($connection,"DELETE FROM stock WHERE quantity=0");
        }
      }
  }
	   ?>
